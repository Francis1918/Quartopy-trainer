...
# If True and STARTING_NET is None, load the newest checkpoint in CHECKPOINT_FOLDER.
...
CHECKPOINT_FOLDER_PATH = CHECKPOINTS_ROOT / EXPERIMENT_NAME
CHECKPOINT_FOLDER = str(CHECKPOINT_FOLDER_PATH)
...
# Show plots on the first resumed epoch to confirm visualization is active.
PLOT_ON_FIRST_EPOCH = True
...
def _extract_epoch_from_checkpoint(ckpt_path: str) -> int:
    matches = re.findall(r"(?:_E_|epoch_)(\d+)", Path(ckpt_path).stem, flags=re.IGNORECASE)
    return int(matches[-1]) if matches else -1


def _find_latest_checkpoint(folder: Path) -> str | None:
    if not folder.exists():
        return None
    ckpts = list(folder.glob("*.pt"))
    if not ckpts:
        return None
    # Prefer highest epoch number; break ties with modification time.
    latest = max(
        ckpts,
        key=lambda p: (_extract_epoch_from_checkpoint(str(p)), p.stat().st_mtime),
    )
    return str(latest)


if AUTO_RESUME_LATEST and STARTING_NET is None:
    STARTING_NET = _find_latest_checkpoint(CHECKPOINT_FOLDER_PATH)
...
logger.info(f"{AUTO_RESUME_LATEST=}")
logger.info(
    f"Starting checkpoint:\t{STARTING_NET if STARTING_NET is not None else 'None (random init)'}"
)
...
if STARTING_NET is not None:
    ...
    try:
        ...
        loaded_epoch = max(_extract_epoch_from_checkpoint(STARTING_NET), 0)
...
for e in tqdm(
    ...
    current_epoch = loaded_epoch + e + 1
    # load models
    p1 = Quarto_bot(
    ...
    logger.info(f"Using n_last_states={n_last_states} for epoch {current_epoch}")

    if GEN_EXPERIENCE_BY_EPOCH or e == 0:
        logger.info("Generating experience for epoch %d", current_epoch)
        ...
        exp, boards = gen_experience(
            ...
            PROGRESS_MESSAGE=f"{Fore.YELLOW}Generating experience for epoch {current_epoch}{Style.RESET_ALL}",
            COLLECT_BOARDS=True,
        )
        logger.info("Initial experience generated.")
    else:
        replay_buffer.empty()
        logger.info(f"Reusing same previous experience for epoch {current_epoch}")
    ...
    contest_results = run_contest(
        ...
        verbose=False,
        mode_2x2=mode_2x2,
        PROGRESS_MESSAGE=f"{Fore.MAGENTA}Running contest for epoch {current_epoch}{Style.RESET_ALL}",
    )
    logger.info(f"Contest results after epoch {current_epoch}")
    ...
    # ------- PLOT RESULTS -----------
    should_plot = ((e + 1) % FREQ_EPOCH_PLOT_SHOW == 0) or ((e + 1) == EPOCHS)
    if PLOT_ON_FIRST_EPOCH and e == 0:
        should_plot = True
